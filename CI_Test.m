function CI = CI_Test(X,Y,S,D,alpha,cond_indep,varargin)

% Two options are implemented: Oracle CI tests (using d-separation 
% relations in the ground truth, and Fischer's Z transform for Gaussian
% variables. use cond_indep as 'oracle' and 'stat' for these two,
% respectively.
...
% cond_indep: type of the CI test: either 'oracle' or 'stat'
% Optional input: ground truth dag G for performing oracle tests
...
%--------------------------------------------------------------------------
    if strcmp(cond_indep, 'oracle')
        if nargin>=7
            ...
            error("Ground-truth DAG is necessary in the input to perform oracle CI tests");
        end
    else
        n = size(D, 1);
        c = norminv(1-alpha/2);
...
% From here on, we have used a few functions to check d-separation, 
% written by other people, namely Kevin Murphy et al. See
