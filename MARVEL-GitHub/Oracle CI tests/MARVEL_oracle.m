function [G_MARVEL, tests,SC] = MARVEL_oracle(G,Mb)
% Input:
% G: the causal graph
% Mb: Markov boundary of G
%
% Output:
...
for iter=p:-1:1
    ...
    for X=ind
        Mb_X = find(Mb(:,X));
        [isR,M1,M2,M3,N,VS,tests,SC] = IsRemovable(X,...
            Mb_X,G_MARVEL,M1,M2,M3,tests,G,SC);
        if isR
            XX = X;
            break
        end
    ...
    if XX == 0
        ...
        [Mb,G_MARVEL,V,M1,M2,tests,SC] = UpdateGraph...
            (XX,M1,M2,N,VS, G, Mb, G_MARVEL, V, tests,SC);
    end
end
end
...
% Condition 1:
[is_neighbors, sep_sets, M1,M2,tests,SC] = neighbors(X, Mb_X, ...
    M1, M2,tests, G,SC);
...
% Condition 2
[VS, M1,M2,M3,tests,SC] = v_structures(X, Mb_X, E, is_neighbors,...
    sep_sets, M1,M2,M3,tests, G,SC);
...
function [VS,M1,M2,M3,tests,SC] = v_structures...
    (X,Mb_X,E,is_neighbors,sep_sets,M1,M2,M3,tests,G,SC)
...
for i = 1:length(neighbors_X)
    ...
    for j = 1:length(coparents_X)
        ...
        [is_vstructs(i,j), M1,M2,M3,tests,SC] = IsVstructure(X,neighbor, ...
            cp,S, Mb_X,M1,M2,M3,tests,G,SC);
    end
end
...
function [is_vstructure, M1,M2,M3,tests,SC] = IsVstructure...
    (X, Z, T, S,Mb_X, M1,M2,M3,tests, G,SC)
...
for i = 1:length(Mb_X)
    T = Mb_X(i);
    [is_neighbors(i), sep_sets(:,i), M1,M2,tests,SC] = IsNeighbor(X, T,...
        Mb_X, M1, M2,tests, G,SC);
end
end

function [is_neighbor,sep_set,M1,M2,tests,SC] = IsNeighbor...
    (X,T,Mb_X,M1,M2,tests,G,SC)
...
function [Mb,E,V,M1,M2,tests,SC] = UpdateGraph...
    (XX,M1,M2,N,VS,G,Mb,E,V,tests,SC)
